...
curl -s $BASE_URL/company -d '{"name": "Discord"}' -H "Content-Type: application/json" | jq . | tee /tmp/test_new_company_for_oppo.json
...
echo "Fail to update w/o company_id"
...
echo "Update"
curl -s $BASE_URL/opportunity/$NEW_ID -X PUT -d '{"id": "'"$NEW_ID"'", "company_id": "'"$COMPANY_ID"'", "name": "Engineering Manager Ads Infra", "process": null}' -H "Content-Type: application/json" | jq .
