...
#curl -s $BASE_URL/opportunity/$NEW_ID -X PUT -d '{"id": "'"$NEW_ID"'", "position": "No company", "process_id": "'"$PROCESS_ID"'"}' -H "Content-Type: application/json" | jq .
curl -s $BASE_URL/opportunity/$NEW_ID -X PUT -d '{"id": "'"$NEW_ID"'", "position": "No company"}' -H "Content-Type: application/json" | jq .
