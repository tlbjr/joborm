...
echo "Not found"
curl $BASE_URL/opportunity/0 | jq .
...
echo "Create new"
curl -s $BASE_URL/opportunity -d '{"company_id": "1", "position": "Something New", "process": {"items": []}}' -H "Content-Type: application/json" | jq .

echo "Fail to update"
curl -s $BASE_URL/opportunity/4 -X PUT -d '{"id": "7", "name": "New Corp"}' -H "Content-Type: application/json" | jq .

echo "Update"
curl -s $BASE_URL/opportunity/4 -X PUT -d '{"id": "4", "company_id": "4", "position": "N/A", "process": {"items": []}}' -H "Content-Type: application/json" | jq .
