...
#echo "Update"
#curl -s $BASE_URL/process/2 -X PUT -d '{"id": "2", "company_id": "2", "position": "N/A", "process": {"items": [{"type_": "Panel", "location": "in-person", "with_": "internal"}]}}' -H "Content-Type: application/json" | jq .
...
curl -s $BASE_URL/opportunity -d '{"position": "Test Process Oppo", "company_id": "'"$COMPANY_ID"'"}' -H "Content-Type: application/json" | jq . | tee /tmp/new_oppo_for_process.json
...
#curl -s $BASE_URL/process/4 -X PUT -d '{"id": "4", "company_id": "1", "position": "N/A", "process": {"items": []}}' -H "Content-Type: application/json" | jq .

#echo "Update Fail - Not Found"
#curl -s $BASE_URL/process/019a3b79-1243-7ca3-9f9c-1adc8b909dfb -X PUT -d '{"id": "019a3b79-1243-7ca3-9f9c-1adc8b909dfb", "company_id": "1", "position": "N/A", "process": {"items": []}}' -H "Content-Type: application/json" | jq .
